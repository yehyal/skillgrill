...
export function AnalyticsPreference() {
  ...
  const isBrowserBlocked = preference === "browser_blocked"
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
        <div>
          <p className="text-sm font-medium" role="status" aria-live="polite">
            Anonymous analytics is {isUnavailable
              ? "not active in this environment"
              : isBrowserBlocked
                ? "disabled by your browser privacy setting"
                : isDisabled
                  ? "disabled"
                  : "enabled"}.
          </p>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {isBrowserBlocked
              ? "Skill Grill respects your browser’s Do Not Track preference."
        ...
        <Button
          ...
          disabled={isUnavailable || isBrowserBlocked}
        >
          {isUnavailable
